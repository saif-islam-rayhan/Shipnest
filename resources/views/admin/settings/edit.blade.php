...
@php
    ...
    $pwd = fn(string $key) => ($hasSecure[$key] ?? false) ? '•••••••• (saved — leave blank to keep)' : '';
@endphp
...
<div class="bg-white rounded-xl ring-1 ring-gray-200 p-6 max-w-lg">
    ...
    <form action="{{ route('admin.settings.maintenance') }}" method="POST" class="space-y-3">@csrf
        ...
        <input name="secret" placeholder="Bypass secret (optional)" class="input-field" value="shipnest-admin">
...
<form action="{{ route('admin.settings.update') }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-xl ring-1 ring-gray-200 p-6 max-w-3xl space-y-4">@csrf @method('PUT')
    ...
    @if($tab==='general')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div><label class="text-sm text-gray-600">Site Name</label><input name="site_name" value="{{ $general['site_name'] ?? '' }}" class="input-field"></div>
            <div><label class="text-sm text-gray-600">App URL</label><input name="app_url" value="{{ $general['app_url'] ?? '' }}" class="input-field" placeholder="https://shipnest.com"></div>
            <div><label class="text-sm text-gray-600">Timezone</label><input name="app_timezone" value="{{ $general['app_timezone'] ?? 'Asia/Dhaka' }}" class="input-field"></div>
            <div><label class="text-sm text-gray-600">Currency</label><input name="currency" value="{{ $general['currency'] ?? 'BDT' }}" class="input-field"></div>
            <div><label class="text-sm text-gray-600">Currency Symbol</label><input name="currency_symbol" value="{{ $general['currency_symbol'] ?? '৳' }}" class="input-field"></div>
            <div><label class="text-sm text-gray-600">Free Shipping Threshold</label><input name="free_shipping_threshold" type="number" value="{{ $general['free_shipping_threshold'] ?? 500 }}" class="input-field"></div>
        </div>
        <h3 class="font-semibold text-gray-800 border-b pb-2 pt-2">Contact</h3>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div><label class="text-sm text-gray-600">Email</label><input name="contact_email" value="{{ $general['contact_email'] ?? '' }}" class="input-field"></div>
            <div><label class="text-sm text-gray-600">Phone</label><input name="contact_phone" value="{{ $general['contact_phone'] ?? '' }}" class="input-field"></div>
            <div class="md:col-span-2"><label class="text-sm text-gray-600">Address</label><textarea name="contact_address" class="input-field" rows="2">{{ $general['contact_address'] ?? '' }}</textarea></div>
        </div>
        <div class="grid grid-cols-2 gap-4">
            <div><label class="text-sm text-gray-600">Logo</label><input type="file" name="logo" accept="image/*" class="text-sm mt-1">@if(!empty($general['logo']))<p class="text-xs text-gray-500 mt-1">{{ $general['logo'] }}</p>@endif</div>
            <div><label class="text-sm text-gray-600">Favicon</label><input type="file" name="favicon" accept="image/*" class="text-sm mt-1"></div>
        </div>
    ...
    @elseif($tab==='database')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div><label class="text-sm text-gray-600">Connection</label><select name="db_connection" class="input-field"><option value="mysql" @selected(($database['db_connection'] ?? 'mysql')==='mysql')>MySQL</option><option value="mariadb" @selected(($database['db_connection'] ?? '')==='mariadb')>MariaDB</option></select></div>
            <div><label class="text-sm text-gray-600">Host</label><input name="db_host" value="{{ $database['db_host'] ?? '127.0.0.1' }}" class="input-field"></div>
            <div><label class="text-sm text-gray-600">Port</label><input name="db_port" value="{{ $database['db_port'] ?? '3306' }}" class="input-field"></div>
            <div><label class="text-sm text-gray-600">Database Name</label><input name="db_database" value="{{ $database['db_database'] ?? '' }}" class="input-field" placeholder="shipnest"></div>
            <div><label class="text-sm text-gray-600">Username</label><input name="db_username" value="{{ $database['db_username'] ?? '' }}" class="input-field"></div>
            <div><label class="text-sm text-gray-600">Password</label><input name="db_password" type="password" class="input-field" placeholder="{{ $pwd('db_password') ?: 'Database password' }}" autocomplete="new-password"></div>
        </div>
    ...
    @elseif($tab==='payment')
        ...
        <h3 class="font-semibold text-gray-800 border-t pt-4">SSLCommerz</h3>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <input name="sslcommerz_store_id" value="{{ $payment['sslcommerz_store_id'] ?? '' }}" placeholder="Store ID" class="input-field">
            <input name="sslcommerz_store_password" type="password" placeholder="{{ $pwd('sslcommerz_store_password') ?: 'Store Password' }}" class="input-field" autocomplete="new-password">
            <input name="sslcommerz_api_url" value="{{ $payment['sslcommerz_api_url'] ?? 'https://sandbox.sslcommerz.com' }}" placeholder="API URL" class="input-field md:col-span-2">
            <label class="flex items-center gap-2 text-sm"><input type="checkbox" name="sslcommerz_sandbox" value="1" @checked(($payment['sslcommerz_sandbox'] ?? '1')==='1')> Sandbox</label>
        </div>
        <p class="text-xs text-gray-500">IPN URL: <code>{{ url('/payment/ipn/sslcommerz') }}</code></p>
    ...
    @elseif($tab==='mail')
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div><label class="text-sm text-gray-600">Mailer</label><select name="mail_mailer" class="input-field"><option value="smtp" @selected(($mail['mail_mailer'] ?? 'smtp')==='smtp')>SMTP</option><option value="log" @selected(($mail['mail_mailer'] ?? '')==='log')>Log (dev)</option></select></div>
    ...
    @elseif($tab==='sms')
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div><label class="text-sm text-gray-600">Driver</label><select name="sms_driver" class="input-field"><option value="mock" @selected(($sms['sms_driver'] ?? 'mock')==='mock')>Mock (dev)</option><option value="twilio" @selected(($sms['sms_driver'] ?? '')==='twilio')>Twilio</option></select></div>
    ...
    @elseif($tab==='commission')
        <label class="text-sm text-gray-600">Default merchant commission rate (%)</label>
