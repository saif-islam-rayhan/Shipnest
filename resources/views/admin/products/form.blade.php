...
<form action="{{ $product->exists ? route('admin.products.update', $product) : route('admin.products.store') }}"
    ...
    <div x-show="step === 5" x-cloak class="bg-white rounded-xl ring-1 ring-gray-200 p-6 space-y-4 pb-24">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 pt-2 border-t">
            <div>
                ...
                <select name="approval_status" class="input-field">
                    @foreach(['approved','pending','rejected'] as $s)
                        <option value="{{ $s }}" @selected(old('approval_status', $product->approval_status ?? 'approved') === $s)>{{ ucfirst($s) }}</option>
                    @endforeach
                </select>
            </div>
            @if($product->exists)
            <div>
                ...
                <select name="status" class="input-field">
                    @foreach(['draft','active','inactive','out_of_stock'] as $s)
                        <option value="{{ $s }}" @selected(old('status', $product->status->value ?? 'draft') === $s)>{{ ucfirst(str_replace('_',' ',$s)) }}</option>
                    @endforeach
                </select>
            </div>
            @endif
